...
const addressValidation = [
  ...
  check("zipCode")
    ...
    .isNumeric()
    .withMessage("Zip code must contain only digits."),

  check("area")
    .trim()
    .notEmpty()
    .withMessage("Please provide area name.")
    .isLength({ min: 2, max: 100 })
  ...
  check("fullAddress")
    ...
    .notEmpty()
    .withMessage(
      "Please provide full address including house number, street name, and nearby landmark.",
    )
...
router.get("/", authCheck, getUserAddress);
router.get("/address_info/:addressId", authCheck, findAddressById);
router.patch("/update/:addressId/default", authCheck, updateIsDefault);
router.patch("/update/:addressId", authCheck, updateAddress);
router.delete("/:addressId", authCheck, deleteAddress);
